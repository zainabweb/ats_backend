...
const EVIDENCE = [
  "listed directly under Skills",
  "used daily in most recent role",
  "mentioned in project description",
  "implied by tools/responsibilities used",
  "named in certification title",
];
...
// Mirrors app/services/rag_service.py exactly: each required skill is judged
// "explicit" (in the CV, verbatim) / "inferred" (implied by projects/tools,
// not written directly) / "missing" against THIS resume - never a fraction of
// the candidate's whole skill list. score = matched / total_required_skills * 100,
// computed here client-side only to preview the shape; the real number always
...
function parseFileToCandidate(file, index) {
  ...
  const detectedTitle = rnd(11) > 0.15 ? "administrative assistant" : "data entry clerk";
  const roleAlignment = detectedTitle === "administrative assistant" ? "aligned" : "different_field";
  ...
  const matchedNames = shuffled.slice(0, matchCount);
  const missing = REQUIRED_SKILLS.filter((s) => !matchedNames.includes(s));

  // Each matched skill is independently "explicit" (exact) or "inferred" (related).
  const matched = matchedNames.map((skill, i) => {
    const isExplicit = rnd(20 + i) > 0.4;
    return {
      skill,
      match_type: isExplicit ? "exact" : "related",
      evidence: isExplicit
        ? EVIDENCE[0]
        : EVIDENCE[1 + Math.floor(rnd(30 + i) * (EVIDENCE.length - 1))],
    };
  });

  // Deterministic - matched / total required skills in the JD, nothing else.
  const score = REQUIRED_SKILLS.length
    ? Math.round((matched.length / REQUIRED_SKILLS.length) * 100)
    : 0;

  const educationMatch = edu !== "High School / Intermediate";
  ...
  return {
    ...
    detectedJobTitle: detectedTitle,
    roleAlignment,
    experience: exp,
    relevantExperience: +(exp * (0.5 + rnd(15) * 0.5)).toFixed(1),
    expSource,
    roleCount,
    education: edu,
    educationMatch,
    ...
    explanation:
      roleAlignment === "aligned"
        ? `Strong overlap on core admin skills; ${matched.length} of ${REQUIRED_SKILLS.length} required skills found, ${matched.filter((m) => m.match_type === "exact").length} stated directly.`
        : `Resume profile reads as '${detectedTitle}', which doesn't overlap with the posted role.`,
    expanded: false,
  };
}
  ...
  const filtered = query
    ? candidates.filter(
        (c) =>
          ...
          c.matched.some((s) => s.skill.toLowerCase().includes(query.toLowerCase()))
      )
    : candidates;

  // Best score first; when two candidates tie on score, more total
  // experience wins - same tie-break rule as rag_service.search().
  const ranked = [...filtered].sort(
    (a, b) => b.score - a.score || b.experience - a.experience
  );
                  ...
                  {ranked.map((c) => (
                    <div key={c.id}>
                      ...
                      >
                        ...
                        <span style={{ color: "#5B5646" }}>{c.detectedJobTitle}</span>
                      ...
                      {c.expanded && (
                        ...
                        >
                          <div>
                            <p className="text-[11px] tracking-widest uppercase mb-2" style={{ color: "#A39D8C" }}>
                              Skills — {c.matched.length} of {REQUIRED_SKILLS.length} matched ({c.score}%)
                            </p>
                            <div className="flex flex-wrap gap-1.5 mb-1">
                              {c.matched.map((s) => (
                                <span
                                  ...
                                  style={
                                    s.match_type === "exact"
                                      ? { background: "#EAF4EC", color: "#2B7A4B" }
                                      : { background: "#EAF1F4", color: "#2B6A7A" }
                                  }
                                >
                                  {s.skill}
                                  <span className="ml-1 opacity-70">
                                    {s.match_type === "exact" ? "· stated" : "· inferred"}
                                  </span>
                                </span>
                              ))}
                            </div>
                            <p className="text-[11px] mb-4" style={{ color: "#A39D8C" }}>
                              "Inferred" = not written directly, but clearly implied by a project/tool on the CV.
                            </p>
                            {c.missing.length > 0 && (
                              <>
                                <p className="text-[11px] tracking-widest uppercase mb-2" style={{ color: "#A39D8C" }}>
                          ...
                          <div>
                            ...
                            <p className="text-xs mb-3" style={{ color: "#A39D8C" }}>
                              Score = matched skills ÷ total skills in the JD. Ties are broken by total experience.
                            </p>
                            <div className="text-sm mb-2" style={{ color: "#1F2A24" }}>
                              <strong>Education</strong> — requires {education}
                              <div className="text-xs" style={{ color: c.educationMatch ? "#2B7A4B" : "#B54B3F" }}>
                                {c.education} {c.educationMatch ? "— meets requirement" : "— below requirement"}
                              </div>
                            </div>
                            <div className="text-sm mb-2" style={{ color: "#1F2A24" }}>
                              <strong>Experience</strong> — requires {minExp}+ yrs
                              <div className="text-xs" style={{ color: "#8A8578" }}>
                                {c.experience} yrs total ({c.relevantExperience} yrs relevant) —{" "}
                                {c.expSource === "stated"
                            ...
                            <div className="text-sm mb-4" style={{ color: "#1F2A24" }}>
                              ...
                              <div className="text-xs" style={{ color: c.roleAlignment === "aligned" ? "#2B7A4B" : "#B54B3F" }}>
                                {c.detectedJobTitle} — {c.roleAlignment === "aligned" ? "aligned with posted role" : "different field"}
                              </div>
                            </div>
                            <p className="text-[11px] tracking-widest uppercase mb-2" style={{ color: "#A39D8C" }}>
                              Why this score
                            </p>
                            <p className="text-xs mb-4" style={{ color: "#5B5646" }}>{c.explanation}</p>
                            <p className="text-[11px] tracking-widest uppercase mb-2" style={{ color: "#A39D8C" }}>
                              Contact
                            </p>
